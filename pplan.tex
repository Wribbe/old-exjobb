...
\newcommand{\qmline}[2][]{%
  ...
  \ifthenelse{\isempty{#1}}%
    {\rule{\textwidth}{0.4pt} }%first is empty.
    {%
      \foreach \index in {1, ..., \numexpr(#1-1)} {%
        \rule{\textwidth}{0.4pt} \\ \\
      }
...
\newcommand{\qyesno}[1]{%
  \textbf{#1}: \\
  \ \hspace{1cm} $\square$ Yes \\
  \ \hspace{1cm} $\square$ No
}
...
\begin{question}
  ...
  \qscale{I am often annoyed by Shotgun}
  \qscale{I always know where in the project process my colleges are}
  \qscale{I think Shtogun provides all functionality I need}
  \qmline[3]{State three positive things about Shotgun:} \\
  \qmline[3]{State three negative things about Shotgun:} \\
%  \qyesno{test}
  \qmline{Approximately, how many hours have you spent in Shotgun?} \\
  \qmline[4]{Comments and Suggestions}
\end{question}
